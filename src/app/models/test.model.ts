...
function parse_response(json: any): LoanAPI {
  let commits: Array<Commit> = [];
  let loan: LoanAPI;
  let mapping_classes: object = {
    "approved_loan": ApprovedLoanCommit,
    "loan_request": LoanRequestCommit,
    "destroyed_loan": DestroyedCommit,
    "lent": LentCommit,
  ...
  for (let commit of json.commits) {
    let c = new mapping_classes[commit.opcode](commit.opcode, commit.timestamp, commit.order, commit.proof, commit.data);
    commits.push(c);
  };
  loan = new LoanAPI(
      json.index,
